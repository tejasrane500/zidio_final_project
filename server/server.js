...
const ServiceAccountKey = require("./react-js-blog-website-69e2c-firebase-adminsdk-fbsvc-28461bbf65.json");
...
server.post("/signup", (req, res) => {
    ...
    //vallidating data from front end
    ...
    if(!passwordRegex.test(password)){
        return res.status(403).json({ "error": "password should be 6 to 20 characters long with a numeric, 1 lowrcase  " })
    }
...
server.post("/signin",(req, res) => {
    ...
    .then((user) => {
        ...
        if(!user.google_auth){
            ...
            if(err) {
                return res.status(403).json({ "error": "Error occured while login please try agains" });
            }
...
server.post("/change-password", verifyJWT, (req, res) => {
    ...
    if(!passwordRegex.test(currentPassword) || !passwordRegex.test(newPassword)){
        return res.status(403).json({ error: "password should be 6 to 20 characters long with a numeric, 1 lowrcase and 1 uppercase letters" })
    }
    ...
    .then((user) => {
        ...
        bcrypt.compare(currentPassword, user.personal_info.password, (err, result) => {
            ...
            bcrypt.hash(newPassword, 10, (err, hashed_password) => {
                ...
                .catch(err => {
                    return res.status(500).json({ error: 'some error occured while saving password' })
                })
...
server.post("/delete-comment", verifyJWT, (req, res) => {
    ...
    .then(comment => {
        ...
        if( user_id == comment.commented_by || user_id == comment.blog ){
...
server.get("/new-notification", verifyJWT, (req,res) => {
    ...
    .then(result => {
        if( result ){
            return res.status(200).json({ new_notification_availlable: true })
        } else{
            return res.status(200).json({ new_notification_availlable: false })
        }
    })
    .catch(err => {
        console.log(err.message);
...
server.post("/notification", verifyJWT, (req, res) => {
    ...
    .sort({ createdAt: -1 })
    .select("createdAt type seen reply comment replied_on_comment blog user")
    .then(notification => {
...
server.post("/delete-blog", verifyJWT, (req, res) => {
    ...
    .then(blog => {
        ...
        Notification.deleteMany({ blog: blog_id }).then(data => console.log('notification deleted'));
